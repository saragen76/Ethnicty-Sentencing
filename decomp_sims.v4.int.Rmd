...
#whites
p.yx0w1 <- seq(0.1,0.9, by=0.05) ##vary
beta_w <- log(p.yx0w1/(1-p.yx0w1)) - beta_0 

#blacks
p.yx1w1 <- p.yx0w1 + 0.05 ##vary
...
tot.size <- length(gamma_x)*length(beta_0)*length(beta_w)*length(beta_wx)
...
l<-1
num.df<-c(p.x1w1[1],p.x0w0[1],p.yx0w1[1],p.yx1w1[1])
...
p_y <- invlogit(beta_0[j] + beta_x * x + beta_w[k] * w + beta_wx[m] * w * x)
...
l<-l+1
num.df <- rbind(num.df,c(p.x1w1[i],p.x0w0[j],p.yx0w1[k], p.yx1w1[m]))
    }
  }
  }
}
...
testy <- unlist(test)
testy<-array(testy,dim=c(tot.size,11,n.reps))
mean.vals <- data.frame(round(apply(testy,c(1,2), mean),3))
colnames(mean.vals) <- c("p.x1w1","p.x0w0","p.yx0w1","p.yx1w1","keep.py","TE_glm", "TE_x_OR", "TE_x", "IE_x", "DE_x", "RES_x")
