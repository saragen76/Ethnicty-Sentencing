...
p.x0w0 <- c(0.4,0.5,0.6) #all plausible were this
beta_0 <- log(p.x0w0/(1-p.x0w0))
...
write.csv(mean.vals, "mean.vals.node.1.csv", row.names = FALSE)
write.csv(sd.vals, "sd.vals.node.1.csv", row.names = FALSE)

#test<-cbind(num.df, test)
plaus <- subset(mean.vals, TE_glm>1.3)
#plaus <- subset(plaus, p.w1 > p.x1w0)
```
...
#beta_x is the direct effect. One possible value is 0, the others are positive
p.x1w0 <- c(0.5,0.6,0.7)
beta_x <- log(p.x1w0/(1-p.x1w0)) - beta_0
...
tot.size <- length(gamma_x)*length(beta_x)*length(beta_w)
...
l<-1
num.df<-c(p.x1w1[1],p.x1w0[1],p.w1[1])
...
l<-l+1
num.df <- rbind(num.df,c(p.x1w1[i],p.x1w0[j],p.w1[k]))
    }
  }
}
num.df<-num.df[-1,]
...
testy<-array(testy,dim=c(tot.size,10,n.reps))
mean.vals <- data.frame(round(apply(testy,c(1,2), mean),3))
colnames(mean.vals) <- c("p.x1w1","p.x1w0","p.w1","keep.py","TE_glm", "TE_x_OR", "TE_x", "IE_x", "DE_x", "RES_x")
sd.vals <- data.frame(round(apply(testy,c(1,2), sd),3))
colnames(sd.vals) <- colnames(mean.vals)

#write.csv(mean.vals, "mean.vals.no.1.csv", row.names = FALSE)
#write.csv(sd.vals, "sd.vals.no.1.csv", row.names = FALSE)
...
```{r}
#test<-cbind(num.df, test)
plaus <- subset(mean.vals, (keep.py > 0.53) & (keep.py < 0.57) & (p.yx1w1 >= p.yx0w1))
plaus <- subset(plaus, p.w1 > p.x1w0)#
plaus <- subset(plaus, (TE_x_OR > 1.45) & (TE_x_OR < 1.6))
